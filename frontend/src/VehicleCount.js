...
const VehicleCounter = () => {
  ...
  useEffect(() => {
    ...
    // Add socket listener for 'vehicle_speed' event
    socket.on('vehicle_speed', (data) => {
      ...
      {showAlert && (
        <div className="mb-4 flex items-center gap-2">
          ...
          <Alert severity="warning">
            <AlertTitle>Warning</AlertTitle>
            {`New detection: ${objType} incoming`}
            {(objType === 'car' || objType==='bus' || objType==='motorcycle') && speed !== null && (
              <p className="text-lg font-bold">Vehicle Speed: {speed.toFixed(2)} meters/second</p>
            )}
          </Alert>
        </div>
      )}
